...
func collectAntigravitySource(reader *bufio.Reader) string {
	...
	switch promptChoice(reader, "Which source should loomWatch use?", options) {
	case 2:
		return "cli"
	case 3:
		return "ide"
...
// collectGrok enables xAI Grok credit tracking. loomWatch auto-detects the bearer
...
func collectGrok(reader *bufio.Reader, logger *slog.Logger) bool {
	fmt.Printf("\n  %sGrok (xAI) Setup%s\n", colorBold, colorReset)
	fmt.Printf("  %sloomWatch reads your Grok login from %s.%s\n", colorDim, api.GrokAuthPath(), colorReset)
...
// collectOpenCode enables ChatGPT tracking via OpenCode's auth.json. loomWatch
...
func collectOpenCode(reader *bufio.Reader) bool {
	...
	fmt.Printf("  %sloomWatch reads your ChatGPT OAuth login from the opencode-codex auth.json.%s\n", colorDim, colorReset)
...
func collectAnthropicToken(reader *bufio.Reader, logger *slog.Logger) string {
	...
	fmt.Printf("  %sloomWatch can auto-detect your Claude Code credentials.%s\n\n", colorDim, colorReset)
...
func collectCodexToken(reader *bufio.Reader, logger *slog.Logger) string {
	...
	fmt.Printf("  %sloomWatch can auto-detect your Codex OAuth token from auth.json.%s\n\n", colorDim, colorReset)
...
func writeEnvFile(path string, cfg *setupConfig) error {
	...
	b.WriteString("# ===============================================================\n")
	b.WriteString("# loomWatch Configuration\n")
	...
	b.WriteString(fmt.Sprintf("LOOMWATCH_ADMIN_USER=%s\n", cfg.adminUser))
	b.WriteString(fmt.Sprintf("LOOMWATCH_ADMIN_PASS=%s\n\n", cfg.adminPass))

	b.WriteString("# Polling interval in seconds (10-3600)\n")
	b.WriteString(fmt.Sprintf("LOOMWATCH_POLL_INTERVAL=%d\n\n", cfg.pollInterval))

	b.WriteString("# Dashboard port\n")
	b.WriteString(fmt.Sprintf("LOOMWATCH_PORT=%d\n", cfg.port))
